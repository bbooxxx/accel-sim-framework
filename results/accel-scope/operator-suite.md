...
|---|---|---|
| GEMV（FFN/decode 代理） | `gemm_bench-tencore inference half 7680 1 2560 0 0` | `inference_half_7680_1_2560_0_0`，kernel-7/8，`cutlass_70_tensorop_h884gemm_128x64_nn_align8` |
...
| 算子 | 计算 | 输入 | 输出 | 数据规模 |
|---|---|---|---|---:|
| GEMV | FP16 `y ← A×x+y`，M=7680, N=1, K=2560，A/B 均不转置 | 矩阵 A: `7680×2560` FP16；向量 x: `2560×1` FP16；初始向量 y: `7680×1` FP16 | 向量 y: `7680×1` FP16 | A/x/y 合计 37.52 MiB |
...
|---|---|---:|---:|---:|---:|---:|
| GEMV | Orin SRAM baseline | 249.470 μs | 14.682 W | 0.70% | 2.13% | 无 L3 |
| GEMV | SCOPE 4-bank | 212.428 μs（降低 **14.85%**） | 10.819 W（降低 **26.31%**） | 0.67% | 2.35% | 99.93% |
...
DeepBench 现有 GEMM trace 中只有这一个不同规模的严格 `N=1` GEMV。其 latency 和
